...
public class EnemyChargeAttack : MonoBehaviour, IAttack
{
    public bool IsAttacking => isAttacking;
    public bool IsOnCooldown => isOnCooldown;
    public bool IsOnAttackLag => isOnAttackLag;
    public float AttackRange => Vector2.Distance(transform.position.ToVector2(), meleeAttackData.hitboxSettings.hitboxEndPos);

    IAttackFX attackFX;
    EnemyStatsComponent stats;
    Animator animator;
    List<PlayerLifeSystem> playerHit = new();
    EnemyController enemyController; //Set controller ref to debug view in editor
    ...
    {
        meleeAttackData = dataRef as MeleeAttackData;
    ...
    {    
        ...
        {
            MeleeHitProcess();
            if (Time.time >= startTime + meleeAttackData.hitboxSettings.hitboxDuration)
            {
    ...
    {
        yield return new WaitForSeconds(meleeAttackData.delay);
    ...
    bool MeleeHitboxCast(out RaycastHit2D[] collisionsList)
    {
        ...
        Vector2 hitboxEndPos = transform.position + currentRotation * meleeAttackData.hitboxSettings.hitboxEndPos;
        ...
        if (meleeAttackData.hitboxSettings.behaviorType != HitboxBehaviorType.Fixed)
        {
            ...
            switch (meleeAttackData.hitboxSettings.behaviorType)
            {
                ...
                case HitboxBehaviorType.MovingOrbital:
                    ...
                    Vector3 currentVector = (Quaternion.AngleAxis(Mathf.LerpAngle(0f, angleValue, meleeAttackData.hitboxSettings.hitboxMovementCurve.Evaluate(t)), transform.forward) * startVector);
                    hitboxCurrentPos = center + currentVector.normalized * Mathf.Lerp(startVector.magnitude, endVector.magnitude, meleeAttackData.hitboxSettings.hitboxMovementCurve.Evaluate(t));
                    break;
            }
        }

        collisionsList = meleeAttackData.hitboxSettings.hitboxShape switch
        {
            HitboxShapeType.Circle => Physics2D.CircleCastAll(hitboxCurrentPos, meleeAttackData.hitboxSettings.circleRadius, Vector2.zero, 0, meleeAttackData.hitboxSettings.activeLayer),
            HitboxShapeType.Box => Physics2D.BoxCastAll(hitboxCurrentPos, meleeAttackData.hitboxSettings.boxSize, Quaternion.Angle(Quaternion.identity, currentRotation), Vector2.zero, 0, meleeAttackData.hitboxSettings.activeLayer),
            _ => null,
        };
    ...
    void MeleeHitProcess()
    {
        if (MeleeHitboxCast(out RaycastHit2D[] collisionsList))
        {
            foreach (RaycastHit2D collision in collisionsList)
            {
                ...
                if (playerLifeSystem && !playerHit.Contains(playerLifeSystem) && playerHit.Count < meleeAttackData.hitboxSettings.maxTargets)
                {
    ...
    {
        cooldownEndTime = Time.time + meleeAttackData.cooldown;
        isOnCooldown = true;
    ...
    void StartAttackLag()
    {
        attackLagEndTime = Time.time + meleeAttackData.lag;
        isOnAttackLag = true;
    }
